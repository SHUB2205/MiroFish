"""
Fish Predictions Backend - Flask Application Factory
"""
...
def create_app(config_class=Config):
    ...
    # Set JSON encoding: ensure non-ASCII characters display directly
    # Flask >= 2.3 uses app.json.ensure_ascii
    ...
    if should_log_startup:
        logger.info("=" * 50)
        logger.info("Fish Predictions Backend starting...")
        logger.info("=" * 50)
    ...
    @app.route('/health')
    def health():
        return {'status': 'ok', 'service': 'Fish Predictions Backend'}
    
    if should_log_startup:
        logger.info("Fish Predictions Backend started successfully")
